...
const OnboardingQuiz: React.FC<{ onSubmit: (profile: Profile) => void }> = ({ onSubmit }) => {
  ...
  return (
    <div className="max-w-2xl p-8 mx-auto mt-10 bg-white rounded-lg shadow-xl">
        ...
        {Object.keys(profile).map((key) => (
          <div key={key}>
            ...
            <input
              ...
              required
              className="w-full px-3 py-2 mt-1 transition-shadow border rounded-md shadow-sm focus:ring-2 focus:ring-opacity-50 focus:border-amber-700 focus:ring-amber-700"
              placeholder={`e.g., ${key === 'major' ? 'Computer Science' : 'Hiking, reading...'}`}
            />
          </div>
        ))}
        <button type="submit" className="w-full py-3 mt-4 font-semibold text-white transition-all rounded-md hover:opacity-90" style={{backgroundColor: LEHIGH_COLORS.brown}}>Find My Clubs</button>
      </form>
    </div>
  );
};
  ...
  return (
    <div className="max-w-5xl p-4 mx-auto md:p-8">
      ...
      <div className="p-6 mb-8 bg-white rounded-lg shadow-lg">
        ...
        {loadingRecs ? (
          <div className="flex flex-col items-center justify-center p-10">
            <div className="w-16 h-16 border-4 rounded-full border-t-transparent animate-spin" style={{borderColor: LEHIGH_COLORS.lightBrown}}></div>
            <p className="mt-4 text-gray-600">Finding the best clubs for you...</p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-3">
            {user.recommendations?.map((rec, index) => {
              ...
              return (
                <div key={index} className="flex flex-col p-4 transition-all duration-300 transform bg-gray-50 rounded-lg shadow-md hover:shadow-xl hover:-translate-y-1" style={{borderColor: LEHIGH_COLORS.gray}}>
                  ...
                  <a href="#" className="mt-4 text-sm font-semibold text-center hover:underline" style={{color: LEHIGH_COLORS.lightBrown}}>
                    Join GroupMe
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <button onClick={() => onNavigate(AppView.COURSE_CATALOG)} className="flex flex-col justify-between p-6 text-left bg-white rounded-lg shadow-lg cursor-pointer group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            <div>
            ...
            <span className="mt-4 font-semibold transition-colors" style={{color: LEHIGH_COLORS.lightBrown}}>View Courses &rarr;</span>
        </button>
        <div className="flex flex-col justify-between p-6 bg-white rounded-lg shadow-lg transition-all duration-300">
            <div>
            ...
            <button disabled className="w-full py-2 mt-4 font-semibold text-white bg-gray-400 rounded-md cursor-not-allowed">Coming Soon</button>
        </div>
      </div>
    </div>
  );
