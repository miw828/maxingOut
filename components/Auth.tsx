...
const Auth: React.FC<AuthProps> = ({ onAuthSuccess }) => {
      ...
      case AuthView.REGISTER:
        return (
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            <button type="submit" className="w-full py-2 text-white rounded-md" style={{backgroundColor: LEHIGH_COLORS.brown, transition: 'background-color 0.3s'}}>Register</button>
            <p className="text-sm text-center">Already have an account? <button type="button" onClick={() => setAuthView(AuthView.LOGIN)} className="font-medium" style={{color: LEHIGH_COLORS.lightBrown}}>Log In</button></p>
          </form>
        );
      case AuthView.FORGOT_PASSWORD:
        return (
          <form onSubmit={handleForgotPassword} className="space-y-6">
            ...
            <button type="submit" className="w-full py-2 text-white rounded-md" style={{backgroundColor: LEHIGH_COLORS.brown, transition: 'background-color 0.3s'}}>Send Reset Link</button>
            <p className="text-sm text-center"><button type="button" onClick={() => setAuthView(AuthView.LOGIN)} className="font-medium" style={{color: LEHIGH_COLORS.lightBrown}}>Back to Login</button></p>
          </form>
        );
      case AuthView.LOGIN:
      default:
        return (
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div/>
              <button type="button" onClick={() => setAuthView(AuthView.FORGOT_PASSWORD)} className="text-sm font-medium" style={{color: LEHIGH_COLORS.lightBrown}}>Forgot password?</button>
            </div>
            <button type="submit" className="w-full py-2 text-white rounded-md" style={{backgroundColor: LEHIGH_COLORS.brown, transition: 'background-color 0.3s'}}>Log In</button>
            <p className="text-sm text-center">Don't have an account? <button type="button" onClick={() => setAuthView(AuthView.REGISTER)} className="font-medium" style={{color: LEHIGH_COLORS.lightBrown}}>Sign Up</button></p>
          </form>
        );
    }
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen" style={{backgroundColor: LEHIGH_COLORS.offWhite}}>
